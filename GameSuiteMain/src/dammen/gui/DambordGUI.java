...
public class DambordGUI implements MouseListener {
	...
	for (int i = 0; i < dambord.getKolommen(); i++) {
		...
		bord.add(
			dambord.getSpeelbord()[dambord.getKolommen() - 1 - i][dambord
				.getRijen() - 1 - j], c);

		dambord.getSpeelbord()[dambord.getKolommen() - 1 - i][dambord
			.getRijen() - 1 - j].addMouseListener(this);
	    }
	}
	...
	for (int i = 0; i < dambord.getKolommen(); i++) {
		...
		if (dambord.getSpeelbord()[i][j].hasDamsteen()) {
		    dambord.getSpeelbord()[i][j]
			    .add(dambord.getSpeelbord()[i][j].getDamsteen());
		}
	    }
	}
